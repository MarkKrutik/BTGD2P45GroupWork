...
public class MoveToPoints : ResetBehaviour
{
    ...
    TagFilteredPlatform tagFilteredTrigger;
    ...
    void Awake()
    {
        ...
        tagFilteredTrigger = GetComponentInChildren<TagFilteredPlatform>();
    ...
    public override void Reset()
    {
        if (resetOnDeath)
        {
            ...
            arrived = false;
            arrivalTime = 0;
            if (tagFilteredTrigger) tagFilteredTrigger.playerOnPlatform = false;
    ...
    void FixedUpdate()
    {
        if (transform.tag != "Enemy")
        {
            ...
            {
                if (tagFilteredTrigger)
                {
                    if (tagFilteredTrigger.colliderActivated)
                        if (!tagFilteredTrigger.playerOnPlatform)
                            return;
                }
